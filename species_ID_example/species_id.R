...
for (time_point in time_points_in_experiment) {
  ...
  nsv = 5 #number of seconds per video
  ...
  morph_mvt = summarize_trajectories(
    ...
    mutate(comment = NULL)  #IMPORTANT: get rid of columns that have empty rows. Otherwise when you use the function complete.cases then you get rid of rows with that column empty.
  ...
  svm1$confusion %>%
    as.data.frame() %>%
    mutate(class.error.percentage = class.error * 100,
           class.error = NULL)
